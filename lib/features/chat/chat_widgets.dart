...
class Bubble extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    return Padding(
      padding: EdgeInsets.only(top: showHeader ? 6 : 1, bottom: 1, left: 8, right: 8),
      child: Align(
              ...
              children: [
                if (message.forwardedFrom != null)
                  Padding(
                    ...
                    child: Text(
                      ...
                      style: TextStyle(fontSize: 11, color: theme.theme.textHint, fontStyle: FontStyle.italic),
                    ),
                  ),
                ...
                if (message.reactions.isNotEmpty)
                  Padding(
                    ...
                    child: ReactionBar(message: message, myId: myId, onTapReaction: () {}),
                  ),
                ...
                Row(
                  ...
                  children: [
                    Text(
                      ...
                      style: TextStyle(
                        fontSize: 11,
                        color: mine ? theme.theme.textHint.withValues(alpha: 0.8) : theme.theme.textHint,
                      ),
                    ),
                    if (mine) ...[
      ...
      child: Column(
        ...
        children: [
          Text(
            ...
            style: const TextStyle(color: GBTheme.whatsAppGreen, fontSize: 11, fontWeight: FontWeight.w700),
          ),
          Text(
        ...
        child: Row(
          ...
          children: [
            ...
            Text('Сообщение удалено',
                style: TextStyle(color: GBTheme.textSecondary, fontStyle: FontStyle.italic, fontSize: 14)),
          ],
        ),
      );
    }
      ...
      case 'CALL':
        return const Row(
          ...
          children: [
            ...
            SizedBox(width: 6),
            Text('Звонок', style: TextStyle(color: GBTheme.textSecondary, fontSize: 14)),
          ],
        );
      default:
  ...
  Widget _formattedText(String text, ThemeProvider theme) {
    ...
    for (final m in matches) {
      if (m.group(2) != null) {
        // ***bold+italic***
        spans.add(TextSpan(text: m.group(2), style: TextStyle(
          fontWeight: FontWeight.bold, fontStyle: FontStyle.italic,
          color: theme.theme.textOther, fontSize: 14.5, height: 1.35)));
      } else if (m.group(3) != null) {
        // **bold**
        spans.add(TextSpan(text: m.group(3), style: TextStyle(
          fontWeight: FontWeight.bold, color: theme.theme.textOther, fontSize: 14.5, height: 1.35)));
      } else if (m.group(4) != null) {
        // *italic*
        spans.add(TextSpan(text: m.group(4), style: TextStyle(
          fontStyle: FontStyle.italic, color: theme.theme.textOther, fontSize: 14.5, height: 1.35)));
      } else if (m.group(5) != null) {
        // ~~strike~~
        spans.add(TextSpan(text: m.group(5), style: TextStyle(
          decoration: TextDecoration.lineThrough, color: theme.theme.textOther, fontSize: 14.5, height: 1.35)));
      } else if (m.group(6) != null) {
        // `mono`
        spans.add(TextSpan(text: m.group(6), style: TextStyle(
          fontFamily: 'monospace', backgroundColor: theme.theme.stroke.withValues(alpha: 0.3),
          color: theme.theme.textOther, fontSize: 14, height: 1.35)));
      } else if (m.group(7) != null) {
        spans.add(TextSpan(text: m.group(7), style: TextStyle(
          color: theme.theme.textOther, fontSize: 14.5, height: 1.35)));
      }
    }
    if (spans.isEmpty) {
      return Text(text, style: TextStyle(fontSize: 14.5, height: 1.35, color: theme.theme.textOther));
    }
  ...
  Widget _mediaPreview(BuildContext context, ThemeProvider theme) {
    ...
    return GestureDetector(
      onTap: () => Navigator.of(context).push(MaterialPageRoute(
        builder: (_) => MediaViewerScreen(imageUrl: resolved, caption: message.text.isEmpty ? null : message.text),
      )),
      child: ClipRRect(
        borderRadius: BorderRadius.circular(8),
        child: SizedBox(
          width: 220,
          child: MediaCachedImage(url: resolved, cacheKey: message.mediaKey ?? message.id, width: 220, height: 180, theme: theme),
        ),
      ),
    );
  }
  ...
  Widget _voiceTile(BuildContext context, ThemeProvider theme) {
    ...
    final url = message.mediaUrl ?? '';
    final resolved = url.startsWith('http') ? url : '${AppConfig.apiBase}$url';
    return _VoiceBubble(url: resolved, durationMs: dur, theme: theme, isMine: message.senderId == myId);
  }
    ...
    return Row(
      ...
      children: [
        Container(
          ...
          child: const Icon(Icons.insert_drive_file, color: GBTheme.whatsAppGreen),
        ),
        ...
        Column(
          ...
          children: [
            ...
            Text(fileSize(size), style: TextStyle(color: theme.theme.textHint, fontSize: 11)),
          ],
        ),
      ],
    );
...
class ReplyBar extends StatelessWidget {
  ...
  const ReplyBar({super.key, required this.message, required this.myId, required this.onCancel});
        ...
        children: [
          ...
          Expanded(
            child: Column(
              crossAxisAlignment: CrossAxisAlignment.start,
              children: [
                Text(message.senderId == myId ? 'Вы' : (message.sender?.displayName ?? ''),
                    style: const TextStyle(color: GBTheme.whatsAppGreen, fontSize: 12, fontWeight: FontWeight.w700)),
                Text(message.preview(), maxLines: 1, overflow: TextOverflow.ellipsis,
                    style: TextStyle(color: theme.theme.textHint, fontSize: 13)),
              ],
            ),
          ),
          IconButton(icon: Icon(Icons.close, color: theme.theme.textHint), onPressed: onCancel),
        ],
      ),
    );
  }
    ...
    return Container(
      ...
      child: Row(
        children: [
          ...
          const SizedBox(width: 10),
          const Expanded(child: Text('Редактирование…', style: TextStyle(color: GBTheme.whatsAppGreen, fontSize: 13))),
          IconButton(icon: Icon(Icons.close, color: theme.theme.textHint), onPressed: onCancel),
        ],
      ),
    );
...
class _InputBarState extends State<InputBar> with SingleTickerProviderStateMixin {
  ...
  void _onFocusChanged() {
    if (mounted) setState(() { _showFormat = _focusNode.hasFocus && widget.controller.text.isNotEmpty; });
  }
  ...
  Widget build(BuildContext context) {
    ...
    return Column(
      ...
      children: [
        if (_showFormat)
          FormattingToolbar(controller: widget.controller, onChanged: () => setState(() {})),
        Container(
          padding: EdgeInsets.only(left: 8, right: 8, top: 6, bottom: 6 + MediaQuery.of(context).padding.bottom),
          decoration: BoxDecoration(
            color: theme.theme.surface,
            border: Border(top: BorderSide(color: theme.theme.stroke, width: 0.4)),
          ),
          child: _isRecording ? _buildRecordingRow(theme) : _buildInputRow(theme, hasText),
        ),
      ],
    );
  }
        ...
        Expanded(
          child: Container(
            ...
            decoration: BoxDecoration(
              ...
              border: Border.all(color: theme.theme.stroke.withValues(alpha: 0.5)),
            ),
            child: Row(
              crossAxisAlignment: CrossAxisAlignment.end,
              children: [
                IconButton(
                  icon: Icon(Icons.emoji_emotions_outlined, color: theme.theme.textHint, size: 24),
                  onPressed: () {},
                ...
                if (!hasText)
                  IconButton(
                    icon: Icon(Icons.camera_alt_outlined, color: theme.theme.textHint, size: 24),
                    onPressed: widget.onImage,
                    padding: const EdgeInsets.all(8),
                    constraints: const BoxConstraints(minWidth: 36, minHeight: 36),
                  ),
                IconButton(
                  icon: Icon(Icons.attach_file, color: theme.theme.textHint, size: 24),
        ...
        Expanded(
          child: Container(
            ...
            decoration: BoxDecoration(
              color: theme.theme.bg,
              borderRadius: BorderRadius.circular(24),
              border: Border.all(color: GBTheme.whatsAppGreen.withValues(alpha: 0.5)),
            ),
            child: Row(
              children: [
                ...
                ScaleTransition(
                  ...
                  child: AnimatedContainer(
                    ...
                    decoration: BoxDecoration(
                      ...
                      boxShadow: _isCancelRecording ? null : glow(color: GBTheme.whatsAppGreen, blur: 8),
                    ),
                  ),
                ),
                const SizedBox(width: 10),
                Text('$mins:$secs', style: TextStyle(fontSize: 14, fontWeight: FontWeight.w600, color: theme.theme.textMain)),
                const SizedBox(width: 8),
                _AudioLevelBars(level: _audioLevel, color: _isCancelRecording ? Colors.red : GBTheme.whatsAppGreen),
                const SizedBox(width: 8),
                AnimatedSwitcher(
                  duration: const Duration(milliseconds: 200),
                  child: _isCancelRecording
                      ? Icon(Icons.arrow_back_ios, size: 14, key: const ValueKey('cancel'), color: Colors.red.withValues(alpha: 0.7))
                      : Icon(Icons.arrow_back_ios, size: 14, key: const ValueKey('slide'), color: theme.theme.textHint.withValues(alpha: 0.5)),
                ),
                Expanded(
                  child: AnimatedSwitcher(
                    duration: const Duration(milliseconds: 200),
                    child: Text(
                      _isCancelRecording ? 'Отпустите для отмены' : 'Slide to cancel',
                      key: ValueKey(_isCancelRecording),
                      style: TextStyle(fontSize: 12, color: _isCancelRecording ? Colors.red : theme.theme.textHint),
                      overflow: TextOverflow.ellipsis,
                    ),
                  ),
                ),
...
class _VoiceBubble extends StatefulWidget {
  ...
  final bool isMine;
  const _VoiceBubble({required this.url, required this.durationMs, required this.theme, required this.isMine});
      ...
      child: Row(
        children: [
          InkWell(
            onTap: _toggle,
            child: Icon(
              _hasError ? Icons.error_outline : (_playing ? Icons.pause_circle_filled : Icons.play_circle_fill),
            ...
            child: Column(
              ...
              children: [
                ...
                Row(
                  ...
                  children: [
                    Text(
                      _hasError ? 'Ошибка воспроизведения' : _formatDuration(_position),
                      style: TextStyle(fontSize: 10, color: _hasError ? Colors.red : theme.theme.textHint),
                    ),
                    Text(_formatDuration(_duration), style: TextStyle(fontSize: 10, color: theme.theme.textHint)),
                  ],
                ),
              ],
            ),
